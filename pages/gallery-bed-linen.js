...
const AboutPage = () => {
  ...
  const slugData = async () => {
    try {
      setLoading(true);
      const res = await gallerySlugData("gallery-bed-linen");
      setPost(res);
      setLoading(false);
    } catch (error) {
      setLoading(false);
      ...
      {loading ? (
        ...
        <>
          ...
          {banner?._links?.["wp:featuredmedia"]?.map((mediaLink) => {
            console.log("✌️mediaLinkgallery-bed-lenin --->", mediaLink);
            return (
              <SubPageBannerTitle
                key={mediaLink?.href}
                mediaLink={mediaLink?.href}
                pageTitle="Hospitality Products Gallery > Bed Linen"
